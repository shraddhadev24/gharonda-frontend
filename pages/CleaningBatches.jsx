...
import BatchModal from "./components/CleaningBatchModal";
import { getAllCleaningBatches } from "../service/service";
...
const Batches = () => {
  ...
  const fetchBatches = async () => {
    const res = await getAllCleaningBatches();
    if (res.status === 200) {
      const { data } = res.data;
  ...
  const handleAddBatch = () => {
    setBatchNameIndex(batches.length + 1);
    setAddBatch(true);
  ...
  return (
    <ScrollView>
      <View style={[styles.container, styles.inlineLayout]}>
        <View>
          <Text style={styles.title}>Cleaning Batch List</Text>
            ...
            <List.Accordion
              ...
              description={
                <View>
                  ...
                  <Chip mode="flat" compact={true}>{"Cleaning"}</Chip>
